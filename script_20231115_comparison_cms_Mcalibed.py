...
img_output = load_exr("./testfiles/CMS_Shoot_EOTF_Corrected_RE.exr")
img_calibed = load_exr("./testfiles/CMS_Shoot_Inner_Corrected.exr")
...
mat_cms_calibed = np.flipud(img_calibed)
...
vec_cms_calibed = mat_cms_calibed.reshape((15 * 15, 3))[:6 * 6 * 6, :]

# ten_cms_input = np.zeros((6, 6, 6, 3))
# ten_cms_output = np.zeros((6, 6, 6, 3))
# ten_cms_calibed = np.zeros((6, 6, 6, 3))
...
#     ten_cms_calibed[_i, _j, _k, :] = vec_cms_calibed[_count, :]
...
comparison(vec_cms_input, vec_cms_calibed, "calibrated (actual)")
