...
def extract_colors_from_cc(img: np.ndarray, str_title):
...
img_cc_refer = load_exr(r"C:\Dropbox\TOEI\20240806_OUterCal_check\OuterCheck_Ref.exr")
...
mat_extracted_w = extract_colors_from_cc(img_trans_w, "CAL_OuterFrustum_v01_dW.exr")
mat_extracted_r = extract_colors_from_cc(img_trans_r, "CAL_OuterFrustum_v01_dR.exr")
mat_extracted_g = extract_colors_from_cc(img_trans_g, "CAL_OuterFrustum_v01_dG.exr")
mat_extracted_b = extract_colors_from_cc(img_trans_b, "CAL_OuterFrustum_v01_dB.exr")
mat_extracted_ref = extract_colors_from_cc(img_trans_ref, "OuterCheck_Ref.exr")
...
# img_calibrated = func_fit(q_flat, list_mat_s, np.zeros_like(mat_cc), np.ones_like(w_avg)).reshape((3, 4, 6)).transpose((1, 2, 0))
...
# img_comp = np.kron(key_kron, mat_extracted_ref) + np.kron(1 - key_kron, img_calibrated)
